...
void data_task(void *pvParameters)
{
   ...
   while(1)
    {	
			...
			CAN_SEND();
			delay_us(150);//两帧CAN数据最小的时间间隔，若间隔再小下一帧CAN数据将无法发送
...
void data_transition(void)
{
  ...
  //数据校验位计算，模式1是发送数据校验
	...
	Send_AutoCharge_Data.buffer[5] = Allow_Recharge;										//自动回充的状态
...
{	
	...
	else if(Usart_Receive=='m'  )
	{
	 ...
	 if(Flag_PID==2) //Analyze the data //分析数据
	 {
			...
			else if( Receive[3]==0x57 ) Flash_SaveFlag = 1;
				...
				switch(Receive[1])
				 {
					 ...
					 case 0x37:  Servo_Mid=Data;break;
					 case 0x38:  break; 	
				 }
			}
			else if( Receive[1]==0x23 ) //APP上点击“发送所有数据”处理方法
			{
				...
				if( i<=50 ) //数据在可接受范围
				{
					...
					//数据赋值
					RC_Velocity = dataArray[0];
					Velocity_KP = dataArray[1];
					Velocity_KI = dataArray[2];
					Velocity_Smoother_Rate = dataArray[3]/1000;
					Angle_Smoother_Rate = dataArray[4];
					Servo_max = dataArray[5];
					Servo_min = dataArray[6];
					Servo_Mid = dataArray[7];
