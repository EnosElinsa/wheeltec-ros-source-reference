...
{	
	...
	*/
	...
	//检查是否写入成功
//	DEBUG_INFO("ACR设置值：5  ACR当前值：%d\r\n",(FLASH->ACR) & 0x07);
//	DEBUG_INFO("PSIZE设置值：2  PSIZE当前值：%d\r\n",(FLASH->CR)>>8 & 0x03);
...
{
	//擦除扇区
	stmflash_erase_sector( stmflash_get_flash_sector(FLASH_SAVE_ADDR) );
	
	//保存转向最小值
	stmflash_write(FLASH_SAVE_ADDR,(u32*)&Servo_min,1);
	
	//保存转向最大值
	stmflash_write(FLASH_SAVE_ADDR+4*1,(u32*)&Servo_max,1);
	
	//保存舵机零点
	stmflash_write(FLASH_SAVE_ADDR+4*2,(u32*)&Servo_Mid,1);
...
void Read_Flash(void)
{
	...
	tmp = stmflash_read_word( FLASH_SAVE_ADDR );
	if( tmp==0xffffffff )
	{
		//空flash使用默认的初始值
	}
	...
	tmp = stmflash_read_word( FLASH_SAVE_ADDR+4*1 );
	if( tmp==0xffffffff )
	{
		//空flash使用默认的初始值
	}
	...
	tmp = stmflash_read_word( FLASH_SAVE_ADDR+4*2 );
	if( tmp==0xffffffff )
	{
		//空flash使用默认的初始值
	}
