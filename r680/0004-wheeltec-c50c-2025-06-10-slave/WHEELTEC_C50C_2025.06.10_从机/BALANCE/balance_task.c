...
void Balance_task(void *pvParameters)
{
    ...
    while(1)
    {
        ...
        if(SysVal.Time_count>CONTROL_DELAY+230)
        {
			...
			{
				robot_control.command_lostcount=BALANCE_TASK_RATE;
				robot_control.Vx=0,robot_control.Vy=0,robot_control.Vz=0;
			}
			
			//从机只接受CAN控制
			if( Get_Control_Mode(_CAN_Control) )
...
static void ResponseControl(void)
{
	...
	else if ( robot.type == 7 )                     Set_Pwm( robot.MOTOR_A.Output , robot.MOTOR_B.Output , 0 , 0 , robot.SERVO.Output ); //安装测试专用.舵机开环控制
	else if ( robot.type == 8 )                     Set_Pwm(-robot.MOTOR_A.Output ,-robot.MOTOR_B.Output , 0 , 0 , 1500 ); //安装测试专用.让舵机保持在中间位置校准
	...
	//MD36
	if( robot.type<=6 ) Set_Pwm( robot.MOTOR_A.Output, robot.MOTOR_B.Output, robot.MOTOR_C.Output, robot.MOTOR_D.Output,0);
	//MD60
	else                Set_Pwm(-robot.MOTOR_A.Output,-robot.MOTOR_B.Output,-robot.MOTOR_C.Output,-robot.MOTOR_D.Output,0);
...
static void PS2_control(void)
{
	...
	float th_strength = 0.1f;
	#if defined DIFF_CAR
	
	if( robot.type >= 2 ) th_strength = 0.6f;
	
	if( strength>th_strength && SysVal.Time_count>CONTROL_DELAY)
	{
