...
static void CAN1_SendTask(void)
{
	...
	CAN_X_speed=robot_control.Vx*1000;
	CAN_Y_speed=robot_control.Vy*1000;
	CAN_Z_speed=robot_control.Vz*1000;
	...
	if(charger.AllowRecharge==1&&EN==1&&robot_control.FlagStop==0)
	{
		//自动回充模式下，不对低电量判断做任何操作
	}
	else
	{
		if(robot_control.FlagStop==1)
		{
			CAN_SENT[0]=0;
			CAN_SENT[1]=0;
			CAN_SENT[2]=0;
