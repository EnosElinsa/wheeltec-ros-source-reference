...
class turn_on_robot
{
	...
	private:
		...
		ros::Subscriber Recharge_Flag_Sub,SetRgb_Sub,AvoidFlag_Sub;
		ros::Subscriber Security_Sub;
		//The speed topic subscribes to the callback function
		//速度话题订阅回调函数
		void Cmd_Vel_Callback(const geometry_msgs::Twist &twist_aux);              

		//安全等级设置回调函数
		void Security_Callback(const std_msgs::UInt8 &flag);

		//灯带设置回调函数
		void Set_LightRgb_Callback(const std_msgs::UInt8MultiArray &buffer);

		//自动回充状态设置回调函数
		void Recharge_Flag_Callback(const std_msgs::UInt8 &Recharge_Flag);

		//超声波状态设置回调函数
		void RangerAvoidFlag_Callback(const std_msgs::Bool &AvoidFlag);

		//避障处理函数
		geometry_msgs::Twist avoid_obstacle(const geometry_msgs::Twist& original_twist);

		ros::Publisher odom_publisher, imu_publisher, voltage_publisher; //Initialize the topic publisher //初始化话题发布者
		ros::Publisher Charging_publisher, Charging_current_publisher, RED_publisher,ChargingMode_publisher;
		ros::Publisher range_a_publisher_,range_b_publisher_,range_c_publisher_,range_d_publisher_,range_e_publisher_,range_f_publisher_;
		...
		unsigned char Calculate_BCC(const uint8_t* checkdata,uint16_t datalen); //General BCC check function //通用BCC校验函数
        ...
        string usart_port_name, robot_frame_id, gyro_frame_id, odom_frame_id,car_mode; //Define the related variables //定义相关变量
        ...
        RECEIVE_AutoCharge_DATA Receive_AutoCharge_Data;  //自动回充数据结构体
		...
		//自动回充设备相关变量
		uint8_t AutoRecharge=0;
        bool Charging=0;            //Whether the robot is charging the flag bit //机器人是否在充电的标志位
        float Charging_Current=0;   //Charging_Current //充电电流
		uint8_t charge_set_state=0; //底盘自动回充的设置状态
        uint8_t RedCount=0;         //Whether the robot finds the marker bit of infrared signal (charging pile)  //机器人是否寻找到红外信号(充电桩)的标志位 
		int ranger_avoid_flag; //避障是否启用标志位
};
#endif
