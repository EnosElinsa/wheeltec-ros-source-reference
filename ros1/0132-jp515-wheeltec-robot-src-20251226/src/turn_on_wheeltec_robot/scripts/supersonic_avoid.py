...
class distance_avoid():
	def __init__(self):
		...
		# 车型
		self.car_mode = "s200_4wd_robot_single_antenna"
	...
	def Get_distance_callback(self,topic):
		...
		if self.car_mode == 'S300' or self.car_mode == 'S300_without_lebai':
			# 根据超声波的数值，给出避障方案
		...
		elif self.car_mode == 'S300_Mini' or self.car_mode == 'S300_Mini_without_lebai':
			# 根据超声波的数值，给出避障方案
			if self.distanceA < self.min_distance_LR or self.distanceB < self.min_distance or self.distanceC < self.min_distance or self.distanceD < self.min_distance or self.distanceE < self.min_distance_LR :
				# 遇到障碍物开始计时
				if self.avoid_flag == 0:
					self.avoid_flag = 1
					self.begin_time=rospy.Time.now()
				...
				# 给出避障方案：遇到障碍物时，计算两边的离障碍物的距离，往距离大的方向转
				if self.distanceA + self.distanceB  <  self.distanceD + self.distanceE:
					self.turn_right = 1
					self.turn_left = 0
				else:
					self.turn_left = 1
		...
		elif self.car_mode == 's200_4wd_robot_single_antenna'or self.car_mode == 's200_4wd_robot_single_antenna':
			# 根据超声波的数值，给出避障方案
			if self.distanceA < self.min_distance or self.distanceB < self.min_distance or self.distanceC < self.min_distance or self.distanceD < self.min_distance:
				# 遇到障碍物开始计时
				if self.avoid_flag == 0:
					self.avoid_flag = 1
					self.begin_time=rospy.Time.now()
...
if __name__ == '__main__':
	try:
		...
		# 检查车型与距离
		print("car_mode: ",distance_avoid.car_mode)
		print("min_distance: ",distance_avoid.min_distance)
		print("min_distance_LR: ",distance_avoid.min_distance_LR)
