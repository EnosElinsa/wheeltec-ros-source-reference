...
void PointCloudMapping::insertKeyFrame(KeyFrame* kf, cv::Mat& color, cv::Mat& depth,int idk,vector<KeyFrame*> vpKFs)
{
    ...
    std::cerr << "Error: color or depth is empty!" << std::endl;
    return;}
    ...
    // 检查点云是否有效
    if (pointcloude.pcE == nullptr || pointcloude.pcE->points.empty()) {
        cerr << "Error: Generated point cloud is empty or invalid!" << endl;
        return;
    }
...
{
    if (kf == nullptr) {
        std::cerr << "Error: KeyFrame is nullptr!" << std::endl;
        return nullptr;
    }
    if (depth.empty() || color.empty()) {
        std::cerr << "Error: depth or color image is empty!" << std::endl;
        return nullptr;
    }
    if (depth.rows != color.rows || depth.cols != color.cols) {
        std::cerr << "Error: depth and color images have different sizes!" << std::endl;
        return nullptr;
    }
    if (color.channels() != 3) {
        std::cerr << "Error: color image is not a 3-channel image!" << std::endl;
        return nullptr;
    }
