...
#include "lslidar_cx_driver/lslidar_cx_driver.h"
...
namespace lslidar_driver {
    ...
    lslidarDriver::lslidarDriver(ros::NodeHandle &node, ros::NodeHandle &private_nh) : nh(node),
                                                                                       ...
                                                                                       config_vert(true),
                                                                                       sweep_data(
                                                                                               new lslidar_msgs::LslidarScan_cx()) {
        config_vert_num = 0;
        count = 0;
    ...
    bool lslidarDriver::loadParameters() {
        ...
        pnh.param<std::vector<int>>("disable_angle_min_vec", disable_angle_min_vec, {0});
        pnh.param<std::vector<int>>("disable_angle_max_vec", disable_angle_max_vec, {0});
        ...
        if(angle_disable_min<0) angle_disable_min+=36000;
        ROS_INFO("lidar packet size: %d", packet_size);
        ROS_INFO("use gps time or not: %d", use_gps_ts);
        ROS_INFO("disable_angle_range: ");
        if (is_filter)
            for (size_t i = 0; i < disable_angle_min_vec.size() && i < disable_angle_max_vec.size(); i++)
                std::cout << disable_angle_min_vec[i] << "--" << disable_angle_max_vec[i] << std::endl;
        return true;
    }
            ...
            if (sweep_data->points.size() > 0)
            {
                for (j = 0; j < sweep_data->points.size(); ++j)
                {
                    if (is_filter)
                    {
                        ...
                        for (size_t i = 0; i < disable_angle_max_vec.size() && i < disable_angle_min_vec.size(); i++)
                        {

                            if ((sweep_data->points[j].azimuth > (disable_angle_min_vec[i])) &&
                                (sweep_data->points[j].azimuth < (disable_angle_max_vec[i])))
                            {
                                should_skip = true;
                                break;
                            };
                        ...
                        if (should_skip)
                            continue;
                        if (sweep_data->points[j].distance > filter_distance_min &&
...
/*                    float h_angle =
    ...
    bool lslidarDriver::lslidarC16Control(lslidar_msgs::lslidar_control::Request &req,
        ...
        ROS_WARN("--------------------------");
        // sleep(1);
        lslidar_msgs::LslidarPacket_cxPtr packet0(new lslidar_msgs::LslidarPacket_cx);
        packet0->data[0] = 0x00;
        packet0->data[1] = 0x00;
        int rc_msop = -1;
...
/*    bool lslidarC16Driver::checkPacketValidity(const RawPacket *packet) {
    ...
    void lslidarC16Driver::difopPoll() {
        ...
        lslidar_msgs::LslidarPacket_cxPtr difop_packet_ptr(new lslidar_msgs::LslidarPacket_cx);
        while (ros::ok()) {
            // keep reading
 ...
 */
    bool lslidarC16Driver::poll(void) {  // Allocate a new shared pointer for zero-copy sharing with other nodelets.
        lslidar_msgs::LslidarPacket_cxPtr packet(new lslidar_msgs::LslidarPacket_cx());
        ...
        // packet timestamp
        if (use_gps_ts) {
            lslidar_msgs::LslidarPacket_cx pkt = *packet;
        ...
        for (size_t fir_idx = start_fir_idx; fir_idx < end_fir_idx; ++fir_idx) {
            ...
            sweep_data->points.push_back(lslidar_msgs::LslidarPoint_cx());
            lslidar_msgs::LslidarPoint_cx &new_point = sweep_data->points[
        ...
        if (end_fir_idx != SCANS_PER_PACKET) {
            ...
            sweep_data = lslidar_msgs::LslidarScan_cxPtr(new lslidar_msgs::LslidarScan_cx());
