...
namespace lslidar_driver {
    ...
    bool LslidarDriver::initialize() {
        ...
        // 计算可用角度区间
        disableAngles = calculateDisableAngles(disable_angle_min_vec, disable_angle_max_vec);
        // 计算可用区间
        ...
        for (const auto& range : usableAngles) {
            std::cout <<"sart_angle:"<< range.start <<"--"<<"end_angle: "<< range.end << std::endl;
        }
    ...
    // 计算非禁用的角度区间
    std::vector<LslidarDriver::AngleRange> LslidarDriver::calculateDisableAngles(const std::vector<int>& disableMin, const std::vector<int>& disableMax) {
