...
//   static int count,DistanceCount; //Static variable for counting //静态变量，用于计数
//   static uint8_t data_count=0;
//   Stm32_Serial.read(Receive_Data_Pr,sizeof(Receive_Data_Pr)); //Read the data sent by the lower computer through the serial port //通过串口读取下位机发送过来的数据
//   data_count++;
//   if(data_count==43) data_count=0;
//   /*//View the received raw data directly and debug it for use//直接查看接收到的原始数据，调试使用
//   ROS_INFO("%x-%x-%x-%x-%x-%x-%x-%x-%x-%x-%x-%x-%x-%x-%x-%x-%x-%x-%x-%x-%x-%x-%x-%x",
//   Receive_Data_Pr[0],Receive_Data_Pr[1],Receive_Data_Pr[2],Receive_Data_Pr[3],Receive_Data_Pr[4],Receive_Data_Pr[5],Receive_Data_Pr[6],Receive_Data_Pr[7],
//   Receive_Data_Pr[8],Receive_Data_Pr[9],Receive_Data_Pr[10],Receive_Data_Pr[11],Receive_Data_Pr[12],Receive_Data_Pr[13],Receive_Data_Pr[14],Receive_Data_Pr[15],
//   Receive_Data_Pr[16],Receive_Data_Pr[17],Receive_Data_Pr[18],Receive_Data_Pr[19],Receive_Data_Pr[20],Receive_Data_Pr[21],Receive_Data_Pr[22],Receive_Data_Pr[23]);
//   */  
//   Distance_Data.Frame_Header = 0xFA; // distance data head 0xFA
//   Distance_Data.Frame_Tail = 0xFC;  // distance data tail 0xFC  

//   Receive_Data.rx[count] = Receive_Data_Pr[0]; //Fill the array with serial data //串口数据填入数组
//   Distance_Data.rx[DistanceCount] = Receive_Data_Pr[0];

//   Receive_Data.Frame_Header = Receive_Data.rx[0]; //The first part of the data is the frame header 0X7B //数据的第一位是帧头0X7B
//   Receive_Data.Frame_Tail = Receive_Data.rx[23];  //The last bit of data is frame tail 0X7D //数据的最后一位是帧尾0X7D

// //接收完24字节才开始处理超声波数据
// if(data_count>24||data_count==0)
// {
//   if(Receive_Data_Pr[0] == Distance_HEADER || DistanceCount>0)
//     DistanceCount++;
//   else 
//     DistanceCount=0;
// }
...
//   if(DistanceCount==19)
//   {
//     DistanceCount=0;  //Prepare for the serial port data to be refill into the array //为串口数据重新填入数组做准备
//     if(Distance_Data.rx[18] == Distance_TAIL) //Verify the frame tail of the packet //验证数据包的帧尾
//     {
//       check=Check_Sum(17,3);  //BCC check passes or two packets are interlaced //BCC校验通过或者两组数据包交错

//       if(check == Distance_Data.rx[17])  
//       { 
...
//   {
//     // printf("[ STM32  Data ]:%x-%x-%x-%x-%x-%x-%x-%x-%x-%x-%x-%x-%x-%x-%x-%x-%x-%x-%x-%x-%x-%x-%x-%x\n",
//     //   Receive_Data.rx[0],Receive_Data.rx[1],Receive_Data.rx[2],Receive_Data.rx[3],Receive_Data.rx[4],
//     //   Receive_Data.rx[5],Receive_Data.rx[6],Receive_Data.rx[7],Receive_Data.rx[8],Receive_Data.rx[9],
//     //   Receive_Data.rx[10],Receive_Data.rx[11],Receive_Data.rx[12],Receive_Data.rx[13],Receive_Data.rx[14],
//     //   Receive_Data.rx[15],Receive_Data.rx[16],Receive_Data.rx[17],Receive_Data.rx[18],Receive_Data.rx[19],
