...
class AutoRecharger():
	def __init__(self):
		...
		self.Recharger_Flag_pub = rospy.Publisher("robot_recharge_flag", UInt8, queue_size=5)
...
Ctrl+C/c:关闭自动回充功能并退出.    Ctrl+C/c:Quit the program.
	...
	def autoRecharger(self, key):
		...
		if self.robot['Charging']==1:
			if (self.robot['Voltage']>25) or (self.robot['Voltage']<16.5 and self.robot['Voltage']>12.5):
		...
		#电压过低时开启导航自动回充
		if self.robot['Charging']==0:
			if (self.robot['Voltage']>18 and self.robot['Voltage']<20) or (self.robot['Voltage']>8 and self.robot['Voltage']<10):
				time.sleep(0.01)
