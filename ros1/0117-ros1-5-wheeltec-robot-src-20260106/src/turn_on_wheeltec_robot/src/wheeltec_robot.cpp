...
//自动回充数据包接收状态
bool autoRecharge_data_state = false;
//超声波数据包接收状态
bool distance_data_state = false;
...
//避障功能
geometry_msgs::Twist turn_on_robot::avoid_obstacle(const geometry_msgs::Twist& original_twist)
{
  geometry_msgs::Twist avoid_twist = original_twist;
...
void turn_on_robot::Cmd_Vel_Callback(const geometry_msgs::Twist &twist_aux)
{
  ...
  geometry_msgs::Twist avoid_twist = avoid_obstacle(twist_aux);
...
//设置回充状态
void turn_on_robot::Recharge_Flag_Callback(const std_msgs::UInt8 &Recharge_Flag)
{
...
void turn_on_robot::Set_LightRgb_Callback(const std_msgs::UInt8MultiArray &buffer)
{
  if( buffer.data.size()!=4 )
  {
    printf("set rgb failed.DataLenErr,It must be 4. Got %zu elements.\r\n",buffer.data.size());
    return;
  }
  
  uint8_t en_flag = (buffer.data[0] == 1);
  uint8_t r = buffer.data[1];
  uint8_t g = buffer.data[2];
  uint8_t b = buffer.data[3];
  Send_Data.tx[0]=FRAME_HEADER;
...
void turn_on_robot::RangerAvoidFlag_Callback(const std_msgs::Bool &AvoidFlag)
{
  ranger_avoid_flag=AvoidFlag.data;
  if( ranger_avoid_flag==1 )
    cout<<"Ranger Avoid Enable."<<endl;
  else
    cout<<"Ranger Avoid Disable."<<endl;
}

void turn_on_robot::Security_Callback(const std_msgs::UInt8 &flag)
{
  SecurityPLY=flag.data;
  //新版本协议
  Send_Data.tx[0] = FRAME_HEADER; //frame head 0x7B //帧头0X7B
  Send_Data.tx[1] = 0;
...
void turn_on_robot::Publish_Voltage()
{
    ...
    //低电量报警,仅在电池电压区间满足
    if( ((Power_voltage>16&&Power_voltage<20)||(Power_voltage>8&&Power_voltage<10)) && Charging==0 && time_period>=2)
    {
...
//发布底盘回充的设置的模式
void turn_on_robot::Publish_ChargingMode()
{
    std_msgs::Bool msg;
...
void turn_on_robot::Publish_distance()
{
  ...
  range_msg.field_of_view = 0.52;//FOV,弧度
  range_msg.min_range = 0.02;//最小测量值      
  range_msg.max_range = 5.1; //最大测量值

  //同步发布所有超声波数据
  range_msg.header.frame_id = "ultrasound_link_A";
  range_msg.range = distanceA;
  range_a_publisher_.publish(range_msg);

  range_msg.header.frame_id = "ultrasound_link_B";
  range_msg.range = distanceB;
  range_b_publisher_.publish(range_msg);

  range_msg.header.frame_id = "ultrasound_link_C";
  range_msg.range = distanceC;
  range_c_publisher_.publish(range_msg);

  range_msg.header.frame_id = "ultrasound_link_D";
  range_msg.range = distanceD;
  range_d_publisher_.publish(range_msg);

  range_msg.header.frame_id = "ultrasound_link_E";
  range_msg.range = distanceE;
  range_e_publisher_.publish(range_msg);

  range_msg.header.frame_id = "ultrasound_link_F";
...
//通用BCC校验函数
unsigned char turn_on_robot::Calculate_BCC(const uint8_t* checkdata,uint16_t datalen)
{
	unsigned char bccval = 0;
...
bool turn_on_robot::Get_Sensor_Data_New()
{
  ...
  static int count=0,count2=0,count3=0; //Static variable for counting //静态变量，用于计数
  ...
  */  
  ...
  Last_Receive = Receive_Data_Pr[0];//保存上一次数据
  ...
  if( count2 == Distance_DATA_size )
  {
    ...
    if(Distance_Data.rx[Distance_DATA_size-1]==Distance_TAIL && 
      Distance_Data.rx[Distance_DATA_size-2]==Calculate_BCC(Distance_Data.rx,Distance_DATA_size-2) )
    {
  ...
  if( count3 == AutoCharge_DATA_SIZE )
  {
    ...
    if(Receive_AutoCharge_Data.rx[AutoCharge_DATA_SIZE-1]==AutoCharge_TAIL && 
      Receive_AutoCharge_Data.rx[AutoCharge_DATA_SIZE-2]==Calculate_BCC(Receive_AutoCharge_Data.rx,AutoCharge_DATA_SIZE-2)) //确认帧尾   
    {
        transition_16 = 0;
        ...
        RedCount =  Receive_AutoCharge_Data.rx[3]; //红外信号个数
        Charging = Receive_AutoCharge_Data.rx[4];  //小车是否在充电
        charge_set_state = Receive_AutoCharge_Data.rx[5];//底盘自动回充的设置状态
        autoRecharge_data_state = true; //数据成功接收标志位
    }
  }
...
void turn_on_robot::Control()
{
  while(ros::ok())
  {
    ...
    //回充数据包
    if( autoRecharge_data_state == true )
    {
      autoRecharge_data_state = false;
      Publish_Charging();
      Publish_ChargingCurrent();
      Publish_RED();
      Publish_ChargingMode();
    }

    //超声波数据包
    if( distance_data_state == true )
    {
      distance_data_state = false;
      Publish_distance();
    }
...
turn_on_robot::turn_on_robot():Sampling_Time(0),Power_voltage(0)
{
  ...
  private_nh.param<std::string>("car_mode",         car_mode,          "mini_mec"); //车型数据
  private_nh.param<int>        ("ranger_avoid_flag", ranger_avoid_flag, 0);//超声波是否使能

  if( ranger_avoid_flag==1 )
    cout<<"Ultrasonic obstacle avoidance is enabled."<<endl;

  //将car_mode转化为小车,便于后续判断车型模式
  ...
  //自动回充话题发布
  Charging_publisher = n.advertise<std_msgs::Bool>("robot_charging_flag", 10);//是否在充电标志位 bool
  Charging_current_publisher= n.advertise<std_msgs::Float32>("robot_charging_current", 10);//充电电流大小 Float
  RED_publisher     = n.advertise<std_msgs::UInt8>("robot_red_flag", 10); //红外信号 UInt8
  ChargingMode_publisher = n.advertise<std_msgs::Bool>("robot_charging_mode", 1);//底盘是否进入回充模式 bool

  //超声波话题发布
  range_a_publisher_ = n.advertise<sensor_msgs::Range>("ultrasonic_data_A", 10);//sensor_msgs/Range格式
  range_b_publisher_ = n.advertise<sensor_msgs::Range>("ultrasonic_data_B", 10);
  range_c_publisher_ = n.advertise<sensor_msgs::Range>("ultrasonic_data_C", 10);
  range_d_publisher_ = n.advertise<sensor_msgs::Range>("ultrasonic_data_D", 10);
  range_e_publisher_ = n.advertise<sensor_msgs::Range>("ultrasonic_data_E", 10);
  range_f_publisher_ = n.advertise<sensor_msgs::Range>("ultrasonic_data_F", 10);

  //自动回充话题订阅
  Recharge_Flag_Sub  = n.subscribe("robot_recharge_flag",10, &turn_on_robot::Recharge_Flag_Callback, this);//进入自动回充模式标志位
  SetRgb_Sub = n.subscribe("set_rgb_color",10, &turn_on_robot::Set_LightRgb_Callback, this);//设置灯带颜色话题 std_msgs::msg::UInt8MultiArray
  AvoidFlag_Sub = n.subscribe("RangerAvoidFlag",10, &turn_on_robot::RangerAvoidFlag_Callback, this);//超声波避障设置话题 bool

  //安全等级Security_Sub
  Security_Sub = n.subscribe("chassis_security",10,&turn_on_robot::Security_Callback,this);

  //Set the velocity control command callback function
  //速度控制命令订阅回调函数设置
  Cmd_Vel_Sub  = n.subscribe("cmd_vel",     10, &turn_on_robot::Cmd_Vel_Callback, this); 
