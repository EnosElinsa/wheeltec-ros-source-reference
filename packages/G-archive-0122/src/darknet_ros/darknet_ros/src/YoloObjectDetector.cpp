...
void copy_image_to_ipl(const image& im, IplImage* ipl) {
    int h = im.h;
    int w = im.w;
    int c = im.c;
    ...
    for(int y = 0; y < h; ++y){
        for(int x = 0; x < w; ++x){
            for(int k = 0; k < c; ++k){
                float val = im.data[k*w*h + y*w + x];
                int pixel = (int)(val * 255);
                if(pixel > 255) pixel = 255;
                if(pixel < 0) pixel = 0;
                ((uchar*)(ipl->imageData + y*ipl->widthStep))[x*c + (2 - k)] = pixel;
            }
        }
    }
}
...
{
  //new code to solve rqt error
  if (!viewImage_) {
    // 不再调用 show_image_cv(...)，而是手动拷贝图像数据
    image im = buff_[(buffIndex_ + 1) % 3];
    copy_image_to_ipl(im, ipl_);
    // 不显示窗口，也不调用 cvWaitKey()
}


  // old code to Publish image.
