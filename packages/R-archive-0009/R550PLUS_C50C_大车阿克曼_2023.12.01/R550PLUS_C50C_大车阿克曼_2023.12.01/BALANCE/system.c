...
//	uart4_init(230400);  
  ...
  //PS2手柄配置初始化,配置为模拟量模式	
	...
	//根据滑块数据初略判断舵机位置再进行初始化，避免舵机突然归位
	if( Car_Mode==4 || Car_Mode==5 )
	{		
			 if( SLIDE_POSITION>=500&&SLIDE_POSITION<=1000 )    Servo_Mid -= 250,servo_LR_flag=1;
		else if( SLIDE_POSITION>1000 )                          Servo_Mid -= 450,servo_LR_flag=2;
		else if( SLIDE_POSITION<=-500&&SLIDE_POSITION>=-1000 )  Servo_Mid += 250,servo_LR_flag=3;
		else if( SLIDE_POSITION<-1000 )                         Servo_Mid += 450,servo_LR_flag=4;
		else
		{
			servo_reset_Flag=1;//无需进行复位的情况
		}
	}
