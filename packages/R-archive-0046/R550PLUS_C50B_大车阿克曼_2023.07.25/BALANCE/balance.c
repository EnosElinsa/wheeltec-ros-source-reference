...
//					Move_X=0,Move_Y=0,Move_Z=0;
				...
				if(CONTROL_DELAY<Time_count && Time_count<CONTROL_DELAY+150) //Advance 1 seconds to test //前进1秒进行测试
				{
					Drive_Motor(0.2, 0); 
					robot_mode_check(); //Detection function //检测函数
				}
				else if(CONTROL_DELAY+150<Time_count && Time_count<CONTROL_DELAY+180) Drive_Motor(0, 0); //The stop forward control is completed //检测完成停止前进控制
...
void robot_mode_check(void)
{
	...
	if(Car_Mode==3||Car_Mode==5)
	{
		if(abs(MOTOR_A.Motor_Pwm)>4000||abs(MOTOR_B.Motor_Pwm)>4000||abs(MOTOR_C.Motor_Pwm)>4000||abs(MOTOR_D.Motor_Pwm)>4000)   error++;
	  else error=0;
	}
	else 	if(Car_Mode==1)
	{
		if(abs(MOTOR_A.Motor_Pwm)>3500||abs(MOTOR_B.Motor_Pwm)>3500||abs(MOTOR_C.Motor_Pwm)>3500||abs(MOTOR_D.Motor_Pwm)>3500)   error++;
	  else error=0;
	}
	else if(Car_Mode==9) 
	{
		if( (float_abs(MOTOR_A.Motor_Pwm)>3200)|| (float_abs(MOTOR_B.Motor_Pwm)>3200) || (float_abs(MOTOR_C.Motor_Pwm)>3200)|| (float_abs(MOTOR_D.Motor_Pwm)>3200) )   error++;
		else error=0;
	}
	else if(Car_Mode==4) 
	{
	...
	{
	  if(abs(MOTOR_A.Motor_Pwm)>3800||abs(MOTOR_B.Motor_Pwm)>3800||abs(MOTOR_C.Motor_Pwm)>3800||abs(MOTOR_D.Motor_Pwm)>3800)   error++;
	  else error=0;
