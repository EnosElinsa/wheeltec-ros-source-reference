...
void Drive_Motor(float Vx,float Vy,float Vz)
{
	...
	if(Allow_Recharge==1)//自动回充模式下不进行平滑,提高响应速度
	{
...
//					Move_X=0,Move_Y=0,Move_Z=0;
			...
			{   //存在回充装备时，对回充装备的状态进行检测
				...
				//自动回充切换到非自动回充模式时，清空1次目标速度，防止在非回充模式下还保留了充电装备的速度值
				if(last_allow_recharge==1&&Allow_Recharge==0) Move_X=0,Move_Y=0,Move_Z=0;
				last_allow_recharge = Allow_Recharge;
			}
			...
			if( Allow_Recharge==1 )
			{
				...
				{
					Move_X = Recharge_UP_Move_X;
					Move_Y = 0;
					Move_Z = Recharge_UP_Move_Z;//将速度告知从机
					Drive_Motor(Recharge_UP_Move_X,0,Recharge_UP_Move_Z);
				}
				//接收到了红外信号，接收来自回充装备的回充控制命令
				else if (RED_STATE!=0)
				{
					Move_X = Recharge_Red_Move_X;
					Move_Y = 0;
					Move_Z = Recharge_Red_Move_Z;//将速度告知从机
...
{
    if( smooth_control.VX==0&&smooth_control.VZ==0&&smooth_control.VY==0 )
    {
...
//	debug_show_diff = diff;
    ...
    //PWM消除检测
    if( smooth_control.VX !=0.0f || smooth_control.VY != 0.0f || smooth_control.VZ != 0.0f)
    {
