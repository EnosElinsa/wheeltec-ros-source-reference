echo  'KERNEL=="ttyUSB*", ATTRS{idVendor}=="10c4", ATTRS{idProduct}=="ea60",ATTRS{serial}=="0002", MODE:="0777", GROUP:="dialout", SYMLINK+="wheeltec_lidar2"' >/etc/udev/rules.d/wheeltec_controller.rules
echo  'KERNEL=="ttyUSB*", ATTRS{idVendor}=="10c4", ATTRS{idProduct}=="ea60",ATTRS{serial}=="0001", MODE:="0777", GROUP:="dialout", SYMLINK+="wheeltec_lidar1"' >/etc/udev/rules.d/wheeltec_lidar.rules
echo  'KERNEL=="ttyCH343USB*", ATTRS{idVendor}=="1a86", ATTRS{idProduct}=="55d4", MODE:="0777", GROUP:="dialout", SYMLINK+="wheeltec_controller"' >/etc/udev/rules.d/wheeltec_controller2.rules
...
echo  'SUBSYSTEM=="video4linux",ATTR{name}=="USB 2.0 Camera",ATTRS{serial}=="AY2BC1202K8",ATTR{index}=="0",MODE:="0777",SYMLINK+="Astra_Gemini_arm"' >>/etc/udev/rules.d/camera.rules
echo  'SUBSYSTEM=="video4linux",ATTR{name}=="USB 2.0 Camera",ATTRS{serial}=="AY2BC1200D",ATTR{index}=="0",MODE:="0777",SYMLINK+="Astra_Gemini_car"' >>/etc/udev/rules.d/camera.rules
