...
def generate_launch_description():
    # Declare arguments
    args = [
	DeclareLaunchArgument('camera_name', default_value='camera_arm'),  # 相机命名空间/前缀；用于组装话题与frame_id
	DeclareLaunchArgument('depth_registration', default_value='true'),  # 是否将深度注册到彩色相机坐标（D2C对齐）
	DeclareLaunchArgument('serial_number', default_value='AY2M543004C'),  # 设备序列号筛选；空=不按SN筛选
	...
	DeclareLaunchArgument('enable_soft_filter', default_value='false'),  # 启用深度软件滤波（去噪/去散斑）
