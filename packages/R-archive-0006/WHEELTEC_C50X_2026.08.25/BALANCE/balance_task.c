...
int last_pwm;
int pwm_hysteresis(int target_pwm)
{
	if (target_pwm > last_pwm + PWM_HYSTERESIS)
	{
			last_pwm = target_pwm;
	}
	else if (target_pwm < last_pwm - PWM_HYSTERESIS)
	{
			last_pwm = target_pwm;
	}
	return last_pwm;
...
//		robot.SERVO.Target    =  7.3f + 823.4f*Left_Angle + (-440.4f)*pow(Left_Angle,2) + (-28.8)*pow(Left_Angle,3);
		...
		//迟滞处理，计算出当下的PWM，如果PWM变化幅值太小则PWM值不改变，避免舵机抖动
		pwm_temp = Akm_Servo.Mid + Akm_Servo.Bias + robot.SERVO.Target;
		robot.SERVO.Output = pwm_hysteresis(pwm_temp);
		last_pwm = robot.SERVO.Output;
	}
