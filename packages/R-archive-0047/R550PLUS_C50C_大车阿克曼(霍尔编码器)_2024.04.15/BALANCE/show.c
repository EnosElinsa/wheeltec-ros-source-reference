...
void oled_show(void)
{
	if(oled_page!=last_oledpage)
	{
		OLED_Clear();
		OLED_Refresh_Gram();
	}
	last_oledpage = oled_page;
	...
	//	if(Car_Mode_Show>7)Car_Mode_Show=7;
		...
		if( Allow_Recharge )  OLED_ShowString(0,0,"RCM :");
		 ...
		 if( Deviation_gyro[2]<0)  OLED_ShowString(90,0,"-"),OLED_ShowNumber(100,0,-Deviation_gyro[2],3,12);
			...
			#else
		 if( SLIDE_POSITION<0)  OLED_ShowString(90,0,"-"),OLED_ShowNumber(100,0,-SLIDE_POSITION,4,12);
		 else                      OLED_ShowString(90,0,"+"),OLED_ShowNumber(100,0, SLIDE_POSITION,4,12);	
			#endif
	...
	else if(oled_page==2)
	{
		...
		//debug信息
		OLED_ShowNumber(0,50,last_state,3,12);
		
		OLED_ShowNumber(64,50,red_now_state,3,12);

		OLED_Refresh_Gram();	
	}
	else if( oled_page==OLED_MAX_PAGE )//统一最后一页为版本号信息
	{
		OLED_ShowString(0,0,"Version:");
		OLED_ShowString(64,0," ");
		OLED_ShowString(72,0,"1");
		OLED_ShowString(80,0,".");
		OLED_ShowString(88,0,"0");
		OLED_ShowString(96,0,"1");
		
		//航模、ps2信号干扰调试值
//		OLED_ShowNumber(0,0,Remoter_Ch1,4,12);
//		OLED_ShowNumber(0,10,Remoter_Ch2,4,12);
//		OLED_ShowNumber(0,20,Remoter_Ch3,4,12);
//		OLED_ShowNumber(0,30,Remoter_Ch4,4,12);
//		oled_showfloat(PS2_LX,64,0,3,2);
//		oled_showfloat(PS2_LY,64,10,3,2);
//		oled_showfloat(PS2_RX,64,20,3,2);
//		oled_showfloat(PS2_RY,64,30,3,2);
//		OLED_ShowNumber(0,40,show_tim_state,3,12);
//		OLED_ShowNumber(0,50,rc_check,3,12);
...
if(PID_Send==1)
	 ...
	 else	if(flag_show==0)
	 {
	   ...
	   printf("{A%d:%d:%d:%d}$",(u8)Left_Figure,(u8)Right_Figure,Voltage_Show,(int)gyro[2]); 
		 ...
		 //自动回充debug
//		printf("{A%d:%d:%d:%d}$",(u8)last_state,(u8)red_now_state,Voltage_Show,Allow_Recharge); 
	 }
	 else
	 {
