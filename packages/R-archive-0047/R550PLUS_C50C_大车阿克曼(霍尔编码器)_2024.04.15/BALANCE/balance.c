...
//舵机实际零点中值，该值可通过手动调整好舵机零点后，观察OLED显示屏第5行得到
int Servo_Mid = 1500;
...
void Drive_Motor(float Vx,float Vz)
{
    ...
    if(Car_Mode==0||Car_Mode==1||Car_Mode==7||Car_Mode==8||Car_Mode==9) //SENIOR_AKM - 高配阿克曼   测试专用Car_Mode==8
    {
    ...
    else if(Car_Mode==2||Car_Mode==3) //TOP_AKM_BS - 顶配阿克曼小车摆式悬挂
    {
        ...
        {
			if(Allow_Recharge==0)//非自动回充才滤波
			{
				//Smoothing the input speed //对输入速度进行平滑处理
				Smooth_control(Vx, RC_Velocity/20000);
		...
		//未开始积分前，舵机的输出调整为中值
		if( TargetServo==0 ) TargetServo = Servo_Mid;
...
//			Servo=TargetServo;
    ...
    else if(Car_Mode==4||Car_Mode==5) //TOP_AKM_DL - 顶配阿克曼小车独立悬挂
    {
        ...
        if((APP_ON_Flag==1||PS2_ON_Flag==1||Remote_ON_Flag==1||CAN_ON_Flag==1||Usart_ON_Flag==1)&&(Car_Mode==5))
        {
			if(Allow_Recharge==0)//非自动回充才滤波
			{
				Smooth_control(Vx, RC_Velocity/20000); //对输入速度进行平滑处理
...
void Balance_task(void *pvParameters)
{
    ...
    while(1)
    {
        ...
        //获取滑轨位置,代表前轮转角角度
        SLIDE_POSITION=0;
        for(u8 i=0; i<slide_buf_size; i++)
        {
            SLIDE_POSITION+=slide_data[i];
        }
        SLIDE_POSITION = (SLIDE_POSITION/slide_buf_size)-2048;
...
//					Move_X=0,Move_Y=0,Move_Z=0;
			if(Allow_Recharge==1)
			{	
				if( nav_walk ) Drive_Motor(Recharge_UP_Move_X,Recharge_UP_Move_Z);
				else
				{	
					if( RED_STATE!=0 ) Drive_Motor(Recharge_Red_Move_X,Recharge_Red_Move_Z); 
					else Drive_Motor(0,0); 
				}
        ...
        if(Turn_Off(Voltage)==0&&robot_mode_check_flag==0||(Allow_Recharge&&EN&&!Flag_Stop))
        {
            ...
            else if (Car_Mode==8)	Set_Pwm(-MOTOR_A.Motor_Pwm,-MOTOR_B.Motor_Pwm,1500); //测试专用
            else if (Car_Mode==9)	Set_Pwm( MOTOR_A.Motor_Pwm, MOTOR_B.Motor_Pwm,Servo); //MD36
#elif Diff_Car
            //Set different PWM control polarity according to different car models
            //根据不同小车型号设置不同的PWM控制极性
        ...
        if(Flash_SaveFlag)
        {
            Buzzer = 1;
            Flash_SaveFlag = 0;
            taskENTER_CRITICAL();//进入临界，保证数据安全
            Write_Flash();
...
void Set_Pwm(int motor_a,int motor_b,int servo)
{
	...
	{
			 if( servo_LR_flag==1 ) Servo_Mid+=250,servo_LR_flag=0;
		else if( servo_LR_flag==2 ) Servo_Mid+=450,servo_LR_flag=0;
		else if( servo_LR_flag==3 ) Servo_Mid-=250,servo_LR_flag=0;
		else if( servo_LR_flag==4 ) Servo_Mid-=450,servo_LR_flag=0;
	}
	...
	{
		servo_reset_Flag=1;//其他车型不需要复位
	}
	 
	
	if( servo_reset_Flag==0 )//舵机未完成复位,使用此方法控制
	{
		if(EN==1)//急停可中断复位过程,避免出现意外
		{
			if(Car_Mode==0||Car_Mode==1||Car_Mode==9)
			{
				if( Servo_PWM>Servo_Mid ) 
				{
					Servo_PWM-=2;
					if( Servo_PWM<=Servo_Mid ) Servo_PWM = Servo_Mid,servo_reset_Flag=1;
					Servo = Servo_PWM;
				}
				if( Servo_PWM<Servo_Mid )
				{
			...
			{
				if( Servo_PWM1>Servo_Mid ) 
				{
					Servo_PWM1-=2;
					if( Servo_PWM1<=Servo_Mid ) Servo_PWM1 = Servo_Mid,servo_reset_Flag=1;
					Servo = Servo_PWM1;
				}
				if( Servo_PWM1<Servo_Mid )
				{
...
{
    static float Bias,Pwm=0,Last_bias;
    static u8 once=1;
	
	if( servo_reset_Flag==1 && EN==1 ) //只有舵机完成复位后才允许pid积分
	{
		//零点位置初始化
		if(once) once = 0,Pwm = Servo_Mid;
		...
		if(Car_Mode==2||Car_Mode==3)
		{
			...
			if(SlideTarget<0||SlidePosition<-200) Pwm+=0.004*1.4*(Bias-Last_bias)+0.009*1.4*Bias;
		...
		if(Car_Mode==4||Car_Mode==5)
		{
			//The mechanical structure determines that the right turn stroke is longer, so the steering speed should be accelerated
			//机械结构决定右转行程比较长，此时加快转向速度
			if(SlideTarget>0||SlidePosition<200) Pwm+=0.002*1.3*(Bias-Last_bias)+0.006*1.3*Bias;
			else Pwm+=0.002f*(Bias-Last_bias)+0.006f*Bias;
		}
...
void Key(void)
{
    u8 tmp;
    tmp=KEY_Scan(100,0);
    if(tmp==single_click) Allow_Recharge = !Allow_Recharge;
...
#define ERROR_PWM 8000 //pwm预警值
    ...
    if( EN==1 && robot_mode_check_flag==0) //保留可以使用急停开关跳过自检的功能
    {
        ...
        if(float_abs(MOTOR_A.Motor_Pwm)>5500 && check_a<50) robot_mode_check_flag=1,LED_B=0;
        if(float_abs(MOTOR_B.Motor_Pwm)>5500 && check_b<50) robot_mode_check_flag=1,LED_B=0;
        if(float_abs(MOTOR_C.Motor_Pwm)>5500 && check_c<50) robot_mode_check_flag=1,LED_B=0;
        if(float_abs(MOTOR_D.Motor_Pwm)>5500 && check_d<50) robot_mode_check_flag=1,LED_B=0;

        //若存在负数，说明存在方向相反的情况：错误类型为：车型选错、驱动接线错误或编码器接线错误
        if( check_a<-100 ||check_b<-100 ||check_c<-100 ||check_d<-100 ) robot_mode_check_flag=1,LED_G=0;
...
void auto_pwm_clear(void)
{
    ...
    //PWM消除检测
    if( MOTOR_A.Target !=0.0f || MOTOR_B.Target !=0.0f)
    {
