...
class KCFTrackerNode(Node):
    ...
    def __init__(self):
        ...
        if self.show_display:
            try:
                ...
                self.get_logger().debug('本地显示窗口已就绪')
            except cv2.error:
        ...
        self._recovery_fail_count = 0  # [v8] 局部确认连续失败计数
    ...
    def _try_capture_template(self):
        ...
        if quality > 0.6 and quality > self._tmpl_best_quality:
            ...
            self.get_logger().debug(f'模板更新: best quality={quality:.3f}')
    ...
    def _run_tracking(self):
        ...
        if not self.is_tracking:
            if self._recovery_candidate is None:
                ...
                if recovered is not None:
                    ...
                    self._recovery_candidate = (score, r_bbox)
                    self._recovery_candidate_count += 1
                    self._recovery_fail_count = 0  # 重置连续失败计数
                    ...
                    if self._recovery_candidate_count >= 3:
                        ...
                        self._depth_smooth = 0.0
                        self._recovery_protection_until = now_t + 2.0  # v11: 2秒保护期
                        self._update_recent_template_only()
                        self.get_logger().debug(
                            f'恢复成功 (局部确认3帧, '
                            f'combined={score:.2f})')
                    ...
                    # [v8] 局部确认失败: 不立即回阶段1, 容忍连续3次失败
                    # 目标可能就在附近, 只是某帧匹配分数没过或稍有偏移
                    self._recovery_fail_count += 1
                    if self._recovery_fail_count >= 3:
                        # 连续3次失败, 确实丢了, 回阶段1
                        self.get_logger().debug(
                            f'局部确认连续失败{self._recovery_fail_count}次, '
                            f'回全图搜索 (确认{self._recovery_candidate_count}次)')
                        self._recovery_candidate = None
                        self._recovery_candidate_count = 0
                        self._recovery_fail_count = 0
                    # 否则保持候选, 下一帧继续在附近找
        ...
        # [v13 根本修复] 面积突变不再单独触发丢失!
        # 原逻辑: area_bloat=True → 直接判丢 (else分支)
        # 问题: CSRT bbox自然波动大, 目标姿态/尺度变化都会触发, 导致反复丢失
        # 新逻辑: 面积突变只作为内容校验的加速信号, 不单独判丢
        #         只有 size_ok/jump_ok 硬性异常, 或内容校验持续不匹配才判丢
        area_bloat = False
        if (self._prev_bbox is not None and self._init_bbox_area > 0
            ...
            if prev_area > 0:
                ...
                if bloat_ratio > 3.0:
                    area_bloat = True  # 只标记, 不直接判丢

        # [v13] 去掉 not area_bloat: 面积突变不阻止正常跟踪
        if size_ok and jump_ok:
            ...
            if self._tmpl_initial is not None:
                ...
                if cw <= 10 or ch <= 10:
                    ...
                    # [v12 根本修复] 内容校验基准改为 _tmpl_recent + _tmpl_initial 取最大值
                    # 原来只用 _tmpl_initial → 目标姿态变化就判丢 (color掉到0.1)
                    # 现在取 max(recent相似度, initial相似度):
                    #   - 姿态变化时 recent 相似度高 → 不误判
                    #   - 真遮挡时两者都低 → 仍然能检测
                    color_sim = 0.0
                    edge_sim = 0.0
                    for ref_tmpl in [self._tmpl_recent, self._tmpl_initial]:
                        if ref_tmpl is None:
                            continue
                        c_sim = cv2.compareHist(
                            ref_tmpl[2], roi_hist, cv2.HISTCMP_CORREL)
                        roi_gray = cv2.cvtColor(roi, cv2.COLOR_BGR2GRAY)
                        roi_edge = cv2.Canny(roi_gray, 50, 150)
                        roi_edge_rs = cv2.resize(roi_edge,
                            (ref_tmpl[1].shape[1], ref_tmpl[1].shape[0]),
                            interpolation=cv2.INTER_NEAREST)
                        e_sim = cv2.matchTemplate(
                            roi_edge_rs, ref_tmpl[1], cv2.TM_CCOEFF_NORMED)[0][0]
                        # 取两个模板中相似度更高的
                        if c_sim > color_sim:
                            color_sim = c_sim
                        if e_sim > edge_sim:
                    ...
                    # [v12] 阈值回归合理值: color<0.3, edge<0.2
                    # 因为现在基准正确了, 不需要靠放宽阈值来避免误判
                    if color_sim < 0.3 and edge_sim < 0.2:
                        self._content_mismatch_count += 1
                        # 有面积突变时5帧判定, 无面积突变时10帧
                        threshold = 5 if area_bloat else 10
            ...
            if self.is_tracking:
                ...
                # [v13] area_bloat 不在此处报告, 它不单独导致丢失
                if reason:
                    self.get_logger().warn(
                        f'目标丢失: {", ".join(reason)}')
                # 记录丢失前最后深度
    ...
    def _publish_cmd_vel(self):
        """[v10] 回归原始转向逻辑 (老板反馈v9的转向逻辑反着来)

        恢复原版简单的角速度控制:
        - 角速度: 像素偏移归一化 × angular_Kp, 用满 max_angular_speed
        - 不反转角速度, 不收紧限幅, 不加边缘保护
        - 这些"改进"反而导致转向和目标反向
        ...
        if linear_raw < 0:
            linear_raw = max(-self.max_linear_speed * 0.8, linear_raw)

        # --- 角速度: 乘以 0.6 抑制转向过猛 ---
        angle_ratio = (self.img_w / 2.0 - cx) / (self.img_w / 2.0)
        angular_raw = self.angular_Kp * angle_ratio * 0.6
